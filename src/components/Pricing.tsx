...
export default function Pricing() {
  ...
  const handleProceedToPayment = async (e: React.FormEvent) => {
    ...
    const finalCity = city.trim() || area.trim() || "Pune";
    const finalArea = area.trim() || city.trim() || "Client Location";
                  ...
                  {modalStep === 1 && (
                    <form onSubmit={handleProceedToPayment} className="space-y-3.5">
                      ...
                      <div className="space-y-3">
                        ...
                        {/* City / Location (Quick Pills + Locality) */}
                        <div>
                          <label className="block text-[10px] uppercase font-bold text-neutral-400 mb-1.5 flex items-center gap-1 font-sans">
                            <MapPin className="w-3 h-3 text-brand-orange" />
                            City / Shoot Location *
                          </label>
                          <div className="flex flex-wrap gap-1.5 mb-2">
                            {["Pune", "Hyderabad", "Mumbai", "Bengaluru", "Karimnagar", "Other"].map((c) => {
                              const isSelected = city === c || (c === "Other" && city && !["Pune", "Hyderabad", "Mumbai", "Bengaluru", "Karimnagar"].includes(city));
                              return (
                                <button
                                  type="button"
                                  key={c}
                                  onClick={() => {
                                    if (c === "Other") {
                                      if (["Pune", "Hyderabad", "Mumbai", "Bengaluru", "Karimnagar"].includes(city)) {
                                        setCity("");
                                      }
                                    } else {
                                      setCity(c);
                          ...
                          <div className="relative flex items-center">
                            <MapPin className="w-3.5 h-3.5 text-neutral-400 absolute left-3 pointer-events-none" />
                            <input
                              id="pricing-city"
                              type="text"
                              placeholder={city ? `Locality or area in ${city} (e.g. Baner, Hitec City)` : "Enter shoot city or locality..."}
                              value={area || (["Pune", "Hyderabad", "Mumbai", "Bengaluru", "Karimnagar"].includes(city) ? "" : city)}
                              onChange={(e) => {
                                setArea(e.target.value);
                                if (!city || !["Pune", "Hyderabad", "Mumbai", "Bengaluru", "Karimnagar"].includes(city)) {
                                  setCity(e.target.value);
                                }
                              }}
                              className="w-full max-w-full pl-8 pr-3 py-2.5 rounded-xl bg-neutral-900/80 border border-white/10 text-white text-xs focus:outline-none focus:border-brand-orange transition-colors box-border"
                            />
                          </div>
                        </div>
